...
export default function useTransformProjectCoordinator(
  projectCoordinatorPoints: ProjectCoordinatorPoint[] | undefined,
) {
  /**
   * This stitches topics and funding programmes given the institution junction tables to the project coordinators
   * since coordinators are institutions, we can reuse the institution junction tables
   */
  ...
  const transformedPoints = useMemo(() => {
    ...
    return projectCoordinatorPoints.map((item) => ({
      ...item,
      topics: reduceTopics(
        findTopicIds(junctionsTopics, item.coordinator_id),
        topics,
      ),
      funding_programmes: reduceFundingProgrammes(
        findFundingProgrammeIds(junctionsFunding, item.coordinator_id),
        funding,
      ),
    }));
  }, [
