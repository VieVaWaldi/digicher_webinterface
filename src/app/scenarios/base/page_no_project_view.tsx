...
export default function BaseScenario() {
  ...
  // const dataView = mapViewInstitution;
  ...
  const filteredDataView = useMemo(() => {
    if (!mapViewInstitution?.length) return [];
    return mapViewInstitution.filter((p) => {
      return (
  ...
  return (
    <div onClick={() => setSelectedInfo(false)}>
      ...
      <BaseUI
        ...
        titleContent={
          <Title name={"Institutions"} count={filteredDataView?.length} />
        }
